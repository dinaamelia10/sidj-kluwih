...
<aside class="w-64 h-screen flex flex-col sticky top-0 left-0 bg-surface-container-lowest border-r border-outline-variant py-lg px-md gap-sm hidden md:flex">
    ...
    <nav class="flex flex-col gap-sm flex-1">
        ...
        <a href="{{ route('admin.pengaturan') }}" class="flex items-center gap-3 px-4 py-3 transition-all duration-200 {{ request()->routeIs('admin.pengaturan') ? 'text-primary font-bold bg-secondary-container rounded-xl' : 'text-on-surface-variant hover:bg-surface-container-high hover:text-primary' }}">
